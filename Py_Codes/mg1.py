...
def calculate_queue_parameters(lambda_rate, mu, service_distribution, *params):
    ...
    L = rho + (lambda_rate ** 2 * sigma_s2 + rho ** 2) / (2 * (1 - rho))
    W = L / lambda_rate
    L_q = L - rho
    W_q = L_q / lambda_rate
    idle_time = 1-rho
...
def main():
    ...
    print(f"\nPerformance Metrics for M/G/1 Queue:")
    print(f"Average number of customers in the system (Ls): {L:.4f}")
    print(f"Average time a customer spends in the system (Ws): {W:.4f}")
    ...
    print(f"Time the server was busy: {server_utilization}")
